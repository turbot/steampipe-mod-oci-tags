locals {
  untagged_sql = <<EOT
    select
      ...
      case
        when tags is not null then id || ' has tags.'
        else id || ' has no tags.'
      end as reason,
      __DIMENSIONS__
    from
...
locals {
  untagged_sql_tenant   = replace(local.untagged_sql, "__DIMENSIONS__", "tenant_id")
  untagged_sql_compartment   = replace(local.untagged_sql, "__DIMENSIONS__", "compartment_id")
}
...
control "analytics_instance_untagged" {
  ...
  sql         = replace(local.untagged_sql_compartment, "__TABLE_NAME__", "oci_analytics_instance")
}

control "apigateway_api_untagged" {
  title       = "Apigateway apis should be tagged"
  description = "Check if Apigateway apis have at least 1 tag."
  sql         = replace(local.untagged_sql_compartment, "__TABLE_NAME__", "oci_apigateway_api")
}
...
control "autoscaling_auto_scaling_configuration_untagged" {
  ...
  sql         = replace(local.untagged_sql_compartment, "__TABLE_NAME__", "oci_autoscaling_auto_scaling_configuration")
}

control "budget_alert_rule_untagged" {
  title       = "Budget alert rules should be tagged"
  description = "Check if Budget alert rules have at least 1 tag."
  sql         = replace(local.untagged_sql_compartment, "__TABLE_NAME__", "oci_budget_alert_rule")
}

control "budget_budget_untagged" {
  title       = "Budget budgets should be tagged"
  description = "Check if Budget budgets have at least 1 tag."
  sql         = replace(local.untagged_sql_compartment, "__TABLE_NAME__", "oci_budget_budget")
}

control "cloud_guard_detector_recipe_untagged" {
  title       = "Cloud guard detector recipes should be tagged"
  description = "Check if Cloud guard detector recipes have at least 1 tag."
  sql         = replace(local.untagged_sql_compartment, "__TABLE_NAME__", "oci_cloud_guard_detector_recipe")
}
...
control "cloud_guard_managed_list_untagged" {
  ...
  sql         = replace(local.untagged_sql_compartment, "__TABLE_NAME__", "oci_cloud_guard_managed_list")
}
